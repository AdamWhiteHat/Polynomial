...
namespace PolynomialLibrary
{
    public partial class Polynomial<T>
    {
        public static class Field<T>
        {
            ...
            public static Polynomial<T> ExponentiateMod(Polynomial<T> startPoly, T exponent, Polynomial<T> polynomialModulus, T integerModulus)
            {
                Polynomial<T> result = Polynomial<T>.One.Clone();
                if (GenericArithmetic<T>.Equal(exponent, GenericArithmetic<T>.Zero)) { return result; }

                Polynomial<T> A = startPoly.Clone();

                if (!GenericArithmetic<T>.IsWholeNumber(exponent))
                {
                    throw new ArithmeticException("Exponentiation of fractional number not supported at this time (perhaps you would like to make a feature request on the GitHub?).");
                }

                byte[] byteArray;

                TypeCode typeCode = GenericArithmetic<T>.GetTypeCode(typeof(T));

                if (typeCode == TypeCode.Double || typeCode == TypeCode.Decimal)
                {
                    int integerValue = GenericArithmetic<int>.Convert<T>(exponent);
                    byteArray = GenericArithmetic<int>.ToBytes(integerValue);
                }
                else
                {
                    byteArray = GenericArithmetic<T>.ToBytes(exponent);
                }
                ...
                while (i < t)
                {
                    A = Field<T>.ModMod(Polynomial<T>.Square(A), polynomialModulus, integerModulus);
                    if (bitArray[i] == true)
                    {
                        result = Field<T>.ModMod(Polynomial<T>.Multiply(A, result), polynomialModulus, integerModulus);
                    }
                    i++;
                }
            ...
            public static Polynomial<T> ModPow(Polynomial<T> poly, T exponent, Polynomial<T> mod)
            {
                ...
                {
                    return Polynomial<T>.Field<T>.Modulus(Polynomial<T>.Square(poly), mod);
                }

                T exp = GenericArithmetic<T>.Clone(exponent);
                Polynomial<T> @base = poly.Clone();
                Polynomial<T> result = Polynomial<T>.One.Clone();

                while (GenericArithmetic<T>.GreaterThan(exponent, GenericArithmetic<T>.Zero))
                {
                    if (GenericArithmetic<T>.Equals(GenericArithmetic<T>.Modulo(exp, GenericArithmetic<T>.Two), GenericArithmetic<T>.One)) // If exp is odd
                    {
                        result = Polynomial<T>.Field<T>.Modulus(Polynomial<T>.Multiply(result, @base), mod);

                        exp = GenericArithmetic<T>.Decrement(exp);

                        if (GenericArithmetic<T>.Equals(exp, GenericArithmetic<T>.Zero))
                        {
                            break;
                        }
                    }

                    @base = Polynomial<T>.Field<T>.Modulus(Polynomial<T>.Square(@base), mod);
                    exp = GenericArithmetic<T>.Divide(exp, GenericArithmetic<T>.Two);
                }

                return Polynomial<T>.Field<T>.Modulus(result, mod);

                /*	Polynomial<T> total = Polynomial<T>.Square(poly);
				T counter = GenericArithmetic<T>.Subtract(exponent, GenericArithmetic<T>.Two);
				while (GenericArithmetic<T>.NotEqual(counter, GenericArithmetic<T>.Zero)) {	total = Polynomial<T>.Multiply(total, poly);
					if (total.CompareTo(mod) < 0) { total = Field<T>.Modulus(total, mod); }	counter = GenericArithmetic<T>.Decrement(counter);
				} return total;	*/
            }
